...
[v2変更] 豆知識(learn)フィールドを廃止。
代わりにsummary本文の中で、専門用語を初出時に括弧書きで
短く説明する方式に変更(初心者がそのまま読める文章にする)。
"""
import json
...
出力は次のJSONのみ。前置き・後書き・コードブロック記号は一切禁止:
{
  ...
  "summary": "判定理由を日本語で4文以内",
  "key_events": ["21:30 米6月CPI発表", ...]
}

summaryの文体ルール:
- 投資初心者が予備知識なしで読めば理解できる文章にすること
- 専門用語や指標名は初出時にその場で括弧書きの短い説明を添える
  (例: RSI(買われすぎ・売られすぎを測る指標)が40台)
- 本文で使っていない用語の解説は書かないこと
